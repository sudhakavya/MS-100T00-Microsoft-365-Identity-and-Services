...
1. You should still be logged into **LON-DC1-ID** as **ADATUM\\Azureuser Administrator**; if not, then do so now.
...
To facilitate this process, the existing ticketing data has been captured in a spreadsheet and a backup spreadsheet, and your lab service provider has stored these two spreadsheets on **LON-CL1-ID**.
...
1. On **LON-CL1-ID** you should have closed the Edge browser at the end of the prior task. If not, then do so now.
1. Since Holly will be using her PC to perform this task, you will use **LON-CL1-ID** in this role-playing exercise as Holly’s PC rather than the IT Consultant's PC as you did in the prior task.
...
1.If a **Pick an account** window appears, select Holly's account.   
...
1. Leave the browser and all existing tabs open on **LON-CL1-ID** for the next task.
...
1. You should still be signed into **LON-CL1-ID** as the **Administrator**, and you should be logged into Microsoft 365 as Holly Dickson. In your browser, you should still have the tab open from the prior task that displays the **Service Desk Requests** list. If not, then navigate to this list now.
